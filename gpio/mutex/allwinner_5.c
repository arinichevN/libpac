...
#define GPIO_LOCK() lockMutex(&gpio_mutex)
#define GPIO_UNLOCK() unlockMutex(&gpio_mutex)

unsigned int pio_base=0;
size_t mmap_length=0;
void *gpio_buf=NULL;
Mutex gpio_mutex = MUTEX_INITIALIZER;
...
int gpioSetup() {
    ...
    close ( fd );
    makeData();
    if ( !initMutex ( &gpio_mutex ) ) {
        gpio_free();
        putsde ( "failed to initialize gpio mutex\n" );
        return 0;
    }
    putsdo("GPIO with mutex\n");
    return 1;
}
